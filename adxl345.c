...
static const uint8_t ADXL345_ADDR = 0x53; // Use 8-bit address
static const uint8_t ADXL345_ADDR_READ = 0xA6; // Use 8-bit address
static const uint8_t ADXL345_ADDR_WRITE = 0xA7; // Use 8-bit address
...
uint8_t adxlBegin(I2C_HandleTypeDef *i2cdev)
{
	...
	HAL_I2C_Mem_Read(adxlPort, ADXL345_ADDR_READ, ADXL345_REG_DEVID, 1, &a, 1,
					 HAL_MAX_DELAY);
	if (a != ADXL345_DEVICE_ID)
		return 0;

	a = 0;
	HAL_I2C_Mem_Write(adxlPort, ADXL345_ADDR_WRITE, ADXL345_REG_POWER_CTL, 1, &a, 1,
					  HAL_MAX_DELAY);
	HAL_Delay(10);
...
{
	uint8_t buf[6];
	HAL_I2C_Mem_Read(adxlPort, ADXL345_ADDR_READ, ADXL345_REG_DATAX0, 1, buf, 6,
					 HAL_MAX_DELAY);
	*x = (buf[1] << 8 | buf[0]);
	*y = (buf[3] << 8 | buf[2]);
	*z = (buf[5] << 8 | buf[4]);
